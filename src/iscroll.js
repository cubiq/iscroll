...
(function(){
	...
	iScroll = function (el, options) {
		...
		that.options = {
			...
			// Events
			onRefresh: null,
			onBeforeScrollStart: function (e) { e.preventDefault ? e.preventDefault() : e.returnValue=false; },
			onScrollStart: null,
		...
		if (isOldIE) that.scroller.ondragstart = function () {
			return false;
		};
		...
		that._bind(START_EV);
		if (!hasTouch) {
			that._bind(MOUSEOUT_EV, that.wrapper);
	...
	pagesX: [], pagesY: [],
		...
		handleEvent: function (e) {
			...
			switch(e.type) {
				case START_EV:
					if (!hasTouch && e.button-(isOldIE?1:0) !== 0) return;
					that._start(e);
					break;
	 ...
	 */
		destroy: function () {
			...
			if (!that.options.hasTouch) {
				that._unbind(MOUSEOUT_EV, that.wrapper);
